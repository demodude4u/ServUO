        ...
        public void EndCombine(Mobile from, object o)
        {
            if (o is Item)
            {
                if (o is Item && (((Item)o).IsChildOf(from.Backpack) || ((Item)o).IsChildOf(from.BankBox)))
                {
					...
					{
						if ( ProspectorsTool >= m_Limit )
						from.SendMessage( "That tool type is all ready full." );
						...
						else
						{
							...
							if ( ProspectorsTool > m_Limit )
                        {
                           ((ProspectorsTool)o).UsesRemaining = ProspectorsTool - m_Limit;
                           ProspectorsTool = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
							BeginCombine(from);
...
*/					else if ( o is SewingKit )
					{
						if ( SewingKit >= m_Limit )
						from.SendMessage( "That tool type is all ready full." );
						...
						else
						{
							...
							if ( SewingKit > m_Limit )
                        {
                           ((BaseTool)o).UsesRemaining = SewingKit - m_Limit;
                           SewingKit = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
					...
					else if ( o is MortarPestle )
					{
						...
						else
						{
							...
							if ( MortarPestle > m_Limit )
                        {
                           ((BaseTool)o).UsesRemaining = MortarPestle - m_Limit;
                           MortarPestle = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
							BeginCombine(from);
					...
					else if ( o is ScribesPen )
					{
						...
						else
						{
							...
							if ( ScribesPen > m_Limit )
                        {
                           ((BaseTool)o).UsesRemaining = ScribesPen - m_Limit;
                           ScribesPen = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
					...
					else if ( o is MalletAndChisel )
					{
						...
						else
						{
							...
							if ( MalletAndChisel > m_Limit )
                        {
                           ((BaseTool)o).UsesRemaining = MalletAndChisel - m_Limit;
                           MalletAndChisel = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
							BeginCombine(from);
                        ...
                        o is Scorp || o is Saw)
                    {
                        ...
                        else
                        {
                            ...
                            if (Carpentry > m_Limit)
                            {
                                ((BaseTool)o).UsesRemaining = Carpentry - m_Limit;
                                Carpentry = m_Limit;
                            }
                            else ((Item)o).Delete();
                            from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
					...
					else if ( o is FletcherTools )
					{
						...
						else
						{
							...
							if ( FletcherTools > m_Limit )
                        {
                           ((BaseTool)o).UsesRemaining = FletcherTools - m_Limit;
                           FletcherTools = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
							BeginCombine(from);
					...
					else if ( o is TinkerTools )
					{
						...
						else
						{
							...
							if ( TinkerTools > m_Limit )
                        {
                           ((BaseTool)o).UsesRemaining = TinkerTools - m_Limit;
                           TinkerTools = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
                    ...
                    else if (o is Tongs || o is SmithHammer || o is SledgeHammer)
                    {
                        if (BlackSmith >= m_Limit)
                            from.SendMessage("That tool type is all ready full.");
                        else
                        {
                            ...
                            if (BlackSmith > m_Limit)
                            {
                                ((BaseTool)o).UsesRemaining = BlackSmith - m_Limit;
                                BlackSmith = m_Limit;
                            }
                            else ((Item)o).Delete();
                            from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
					...
					else if (o is Shovel || o is SturdyShovel)
					{
						...
						else
						{
							...
							if ( Shovel > m_Limit )
                        {
                           ((BaseHarvestTool)o).UsesRemaining = Shovel - m_Limit;
                           Shovel = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
					...
					else if ( o is Pickaxe || o is SturdyPickaxe )
					{
						...
						else
						{
							...
							if ( Shovel > m_Limit )
                        {
                           ((BaseAxe)o).UsesRemaining = Shovel - m_Limit;
                           Shovel = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
					...
					else if ( o is GargoylesPickaxe )
					{
						...
						else
						{
							...
							if ( GargoylesPickaxe > m_Limit )
                        {
                           ((BaseAxe)o).UsesRemaining = GargoylesPickaxe - m_Limit;
                           GargoylesPickaxe = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
							BeginCombine(from);
					...
					else if (o is Hatchet || o is Axe || o is BattleAxe || o is DoubleAxe || o is ExecutionersAxe || o is LargeBattleAxe || o is TwoHandedAxe)
					{
						if ( Lumberjacking >= m_Limit )
						from.SendMessage( "That tool type is all ready full." );
						...
						else
						{
							...
							if ( Lumberjacking > m_Limit )
                        {
                           ((BaseAxe)o).UsesRemaining = Lumberjacking - m_Limit;
                           Lumberjacking = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
							BeginCombine(from);
					...
					else if (o is RollingPin || o is Skillet || o is FlourSifter)
					{
						...
						else
						{
							...
							if ( Cooking > m_Limit )
                        {
                           ((BaseTool)o).UsesRemaining = Cooking - m_Limit;
                           Cooking = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
					...
					else if (o is MapmakersPen)
					{
						...
						else
						{
							...
							if ( Cartography > m_Limit )
                        {
                           ((BaseTool)o).UsesRemaining = Cartography - m_Limit;
                           Cartography = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
					...
					else if (o is Blowpipe)
					{
						...
						else
						{
							...
							if ( Glassblowing > m_Limit )
                        {
                           ((BaseTool)o).UsesRemaining = Glassblowing - m_Limit;
                           Glassblowing = m_Limit;
						}
							else ((Item)o).Delete();
							from.SendGump(new ToolHouseGump((PlayerMobile)from, this));
							BeginCombine(from);
        ...
        public ToolHouse(Serial serial)
            : base(serial)
        {
        }
...
//						m_TaxidermyKit = reader.ReadInt();
    ...
    public class ToolHouseGump : Gump
    {
        ...
        public ToolHouseGump(PlayerMobile from, ToolHouse house)
            : base(25, 25)
        {
            m_From = from;
            m_House = house;
            ...
            AddPage(0);
            /*
            AddBackground(50, 10, 455, 335, 5054); // bright background
            AddImageTiled(58, 20, 438, 306, 2624); // dark background
            AddAlphaRegion(58, 20, 438, 306);
            */
            AddBackground(50, 10, 455, 400, 5054); // bright background
            AddImageTiled(58, 20, 438, 380, 2624); // dark background
            AddAlphaRegion(58, 20, 438, 380);

            /*
            AddLabel(325, 150, 0x486, "Taxidermy Kit");
            AddLabel(440, 150, 0x480, house.TaxidermyKit.ToString());
            AddButton(275, 150, 4005, 4007, 17, GumpButtonType.Reply, 0);
            AddLabel(325, 125, 0x486, "Glassblowing");
            */

            AddLabel(65, 335, 0x26, "Lumberjacking works with ANY AXE including magical axes & artifacts!");
            AddLabel(65, 355, 0x26, "ALL magical properties are lost after adding tool to box!");
            AddLabel(65, 375, 0x26, "Be careful what axe you click on! USE AT YOUR OWN RISK!!!");

            AddLabel(250, 25, 0x480, "Toolbox Contents");
			...
			AddLabel(325, 50, 0x486, "Lumberjacking");
			AddLabel(440, 50, 0x480, house.Lumberjacking.ToString());
			AddButton(275, 50, 4005, 4007, 12, GumpButtonType.Reply, 0);

            AddLabel(325, 75, 0x486, "Cooking");
            AddLabel(440, 75, 0x480, house.Cooking.ToString());
            AddButton(275, 75, 4005, 4007, 13, GumpButtonType.Reply, 0);

            AddLabel(325, 100, 0x486, "Cartography");
            AddLabel(440, 100, 0x480, house.Cartography.ToString());
            AddButton(275, 100, 4005, 4007, 14, GumpButtonType.Reply, 0);

            AddLabel(325, 125, 0x486, "Glassblowing");
            AddLabel(440, 125, 0x480, house.Glassblowing.ToString());
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            else if (info.ButtonID == 5)
            {
                ...
                {
                    m_From.AddToBackpack(new Saw(m_House.Carpentry));
                    m_House.Carpentry = (0);
                    m_From.SendGump(new ToolHouseGump(m_From, m_House));
                }
                else
			...
			else if (info.ButtonID == 9)
            {
                if (m_House.Shovel > 0)
                {
                    ...
                    pickaxe.UsesRemaining = m_House.Shovel;
...
namespace Server.Items
{
    public class ToolHouseTarget : Target
    {
        ...
        public ToolHouseTarget(ToolHouse house)
