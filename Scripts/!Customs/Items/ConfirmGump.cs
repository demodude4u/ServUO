...
namespace Server.Gumps
{
	...
	public class ConfirmGump : Gump
	{
        ...
        public ConfirmGump(string content, ConfirmGumpCallback callback) :
            this("Confirm!", content, callback)
        {
        }

        public ConfirmGump(string header, string content, ConfirmGumpCallback callback) :
            this(header, 0xFFEEDD, content, 0xFFEEDD, 300, 300, callback)
        {
        }

        public ConfirmGump(string header, int headerColor, string content, int contentColor, int width, int height, ConfirmGumpCallback callback) :
            base(Math.Max(((800 - width) / 2), 40), Math.Max(((600 - height) / 2), 40))
        {
            m_Callback = callback;
