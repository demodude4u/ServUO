...
{
	public class StaffVariableCarpetAddonDeed : BaseAddonDeed
	{
		public override BaseAddon Addon{ get{ return null; } }
		
		[Constructable]
        public StaffVariableCarpetAddonDeed()
		{
			Name = "Variable Carpet Addon Deed (Staff Version)"; 
			ItemID = 0x3F08;           
		}

		public StaffVariableCarpetAddonDeed( Serial serial ) : base( serial )
		{
		}
		...
		private void BoundingBox_Callback( Mobile from, Map map, Point3D start, Point3D end, object state )
		{
			...
			if ( width < 2 || height < 2 )
				...
				from.SendGump( new StaffVariableCarpetGump( this, p, map, width, height ) );
			else
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
		}
...
{
	public class StaffVariableCarpetGump : Gump
	{
		...
		public StaffVariableCarpetGump( BaseAddonDeed deed, IPoint3D p, Map map, int width, int height ) : base( 30, 30 )
		{
			m_Deed = deed;
			m_P3D = p;
			m_Map = map;
			...
			for ( int i = 0, index = 0; i < StaffVariableCarpetInfo.Infos.Length; ++i, ++index )
			{
				if ( index >= EntryCount )
				{
					...
					if ( (StaffVariableCarpetInfo.Infos.Length - (EntryCount * page)) < EntryCount )
						AddImage( 420, 16, 0x2622 );
				}
				
				StaffVariableCarpetInfo info = StaffVariableCarpetInfo.GetInfo( i );
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID >= 1 )
			{
				BaseAddon addon = new StaffVariableCarpetAddon( info.ButtonID-1, m_Width, m_Height );
				...
				{
				addon.MoveToWorld( new Point3D( m_P3D ), m_Map );
				from.SendMessage( "Carpet placed" );
				}
			}
		}
	}
