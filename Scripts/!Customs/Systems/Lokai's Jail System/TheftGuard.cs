...
{
	public class TheftGuard : CrimeGuard
	{
		...
		public TheftGuard(Mobile target)
			: base(target)
		{
			Title = "the guard";
		...
		public TheftGuard(Serial serial)
			: base(serial)
		{ }
		...
		{
			private readonly TheftGuard m_Owner;

			public AttackTimer(TheftGuard owner)
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					if (target is PlayerMobile)
					{
						...
						string message = "You have been charged with Theft. Will you accept punishment and go to jail willingly?";
		...
		private static void ArrestConfirm_Callback( Mobile guard, Mobile from, bool okay )
		{
			...
			if ( okay )
			{
				JailUtility.NewJailStatus(guard, from, 20);
			}
			else
			{
				JailUtility.IssueWarrant(from, guard, "Theft", DateTime.UtcNow, from.Location, from.Map);
				guard.Say("So be it.");
		...
		private class IdleTimer : Timer
		{
			...
			public IdleTimer(TheftGuard owner)
