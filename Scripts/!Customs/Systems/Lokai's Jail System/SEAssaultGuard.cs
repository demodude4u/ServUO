...
{
	public class SEAssaultGuard : CrimeGuard
	{
		private Timer m_AttackTimer, m_IdleTimer;
		private Mobile m_Focus;

		[Constructable]
		public SEAssaultGuard()
			: this(null)
		{ }

		public SEAssaultGuard(Mobile target)
			: base(target)
		{
			Title = "the guard";
		...
		public SEAssaultGuard(Serial serial)
			: base(serial)
		{ }
		...
		{
			private readonly SEAssaultGuard m_Owner;
			//	private bool m_Shooting;
			public AttackTimer(SEAssaultGuard owner)
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					{
						string header = "Wilt thou come quietly?";
						string message = "You have been charged with Assault. Will you accept punishment and go to jail willingly?";
						target.CloseGump(typeof(ConfirmWarrantGump));
						target.SendGump( new ConfirmWarrantGump( m_Owner, header, 32767, message, 32767, 300, 300, new ConfirmWarrantGumpCallback( ArrestConfirm_Callback ) ) );
						target.Blessed = true;
						target.Frozen = true;
					}
					else
		...
		private static void ArrestConfirm_Callback( Mobile guard, Mobile from, bool okay )
		{
			from.Blessed = false;
			from.Frozen = false;
			if ( okay )
			{
				JailUtility.NewJailStatus(guard, from, 30);
			}
			else
			{
				JailUtility.IssueWarrant(from, guard, "Assault", DateTime.UtcNow, from.Location, from.Map);
				guard.Say("So be it.");
				guard.Combatant = from;
			}
		}
		...
		private class IdleTimer : Timer
		{
			...
			public IdleTimer(SEAssaultGuard owner)
