...
{
	public class MurderGuard : CrimeGuard
	{
		...
		public MurderGuard(Mobile target)
			: base(target)
		{
			Title = "the guard";
		...
		public MurderGuard(Serial serial)
			: base(serial)
		{ }
		...
		{
			private readonly MurderGuard m_Owner;
			//	private bool m_Shooting;
			public AttackTimer(MurderGuard owner)
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					if (target is PlayerMobile)
					{
						...
						string message = "You have been charged with Murder. Will you accept punishment and go to jail willingly?";
		...
		private static void ArrestConfirm_Callback( Mobile guard, Mobile from, bool okay )
		{
			...
			if ( okay )
			{
				JailUtility.NewJailStatus(guard, from, 60);
			}
			else
			{
				JailUtility.IssueWarrant(from, guard, "Murder", DateTime.UtcNow, from.Location, from.Map);
				guard.Say("So be it.");
		...
		private class IdleTimer : Timer
		{
			...
			public IdleTimer(MurderGuard owner)
