...
namespace Server.Customs.Invasion_System
{
    public class TownInvasion
    {
        ...
        private Map _SpawnMap = Map.Trammel;
        ...
        public void OnStart()
        {
            if (!IsRunning)
            {
                ...
                switch (invading)
                {
                    case InvasionTowns.BritainWest:
                        {
                            Top = new Point3D(1413, 1541, 30);
                            Bottom = new Point3D(1487, 1761, -2);
                            MinSpawnZ = 50; //Not sure about this one - don't want them in the tunnels
                            MaxSpawnZ = 61;
                            SpawnMap = Map.Trammel;
                            TownInvaded = "West Britain";
                            break;
                        }
                    case InvasionTowns.BritainEast:
                        {
                            Top = new Point3D(1555, 1528, 40);
                            Bottom = new Point3D(1659, 1704, 13);
                            MinSpawnZ = 50; //Not sure about this one - don't want them in the tunnels
                            MaxSpawnZ = 61;
                            SpawnMap = Map.Trammel;
                            TownInvaded = "East Britain";
                            break;
                        }
                ...
                {
                    string townInvaded = TownInvaded;

                    if(TownInvaded == "East Britain" || TownInvaded == "West Britain")
                    {
                        townInvaded = "Britain";
                    }
        ...
        private void Spawn()
        {
            ...
            switch (_TownMonsterType)
            {
                ...
                case TownMonsterType.UndeadNewbie: entries = MonsterTownSpawnEntry.UndeadNewbie; break;
            }
