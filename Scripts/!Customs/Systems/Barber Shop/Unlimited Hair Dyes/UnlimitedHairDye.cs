...
{
	public class UnlimitedHairDye : Item
	{
		public override int LabelNumber{ get{ return 1041060; } } // Hair Dye

		[Constructable]
		public UnlimitedHairDye() : base( 0xEFF )
		{
			Name = "Hair Dye [Unlimited Use]";
			Weight = 1.0;
		}

		public UnlimitedHairDye( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.CloseGump( typeof( UnlimitedHairDyeGump ) );
				from.SendGump( new UnlimitedHairDyeGump( this ) );
			}
	...
	public class UnlimitedHairDyeGump : Gump
	{
		private UnlimitedHairDye m_UnlimitedHairDye;

		private class UnlimitedHairDyeEntry
		{
			...
			public UnlimitedHairDyeEntry( string name, int hueStart, int hueCount )
			{
				m_Name = name;
				m_HueStart = hueStart;
				m_HueCount = hueCount;
		...
		private static UnlimitedHairDyeEntry[] m_Entries = new UnlimitedHairDyeEntry[]
			{
				new UnlimitedHairDyeEntry( "*****", 1602, 26 ),
				new UnlimitedHairDyeEntry( "*****", 1628, 27 ),
				new UnlimitedHairDyeEntry( "*****", 1502, 32 ),
				new UnlimitedHairDyeEntry( "*****", 1302, 32 ),
				new UnlimitedHairDyeEntry( "*****", 1402, 32 ),
				new UnlimitedHairDyeEntry( "*****", 1202, 24 ),
				new UnlimitedHairDyeEntry( "*****", 2402, 29 ),
				new UnlimitedHairDyeEntry( "*****", 2213, 6 ),
				new UnlimitedHairDyeEntry( "*****", 1102, 8 ),
				new UnlimitedHairDyeEntry( "*****", 1110, 8 ),
				new UnlimitedHairDyeEntry( "*****", 1118, 16 ),
				new UnlimitedHairDyeEntry( "*****", 1134, 16 )
			};

		public UnlimitedHairDyeGump( UnlimitedHairDye dye ) : base( 50, 50 )
		{
			m_UnlimitedHairDye = dye;
			...
			for ( int i = 0; i < m_Entries.Length; ++i )
			{
				UnlimitedHairDyeEntry e = m_Entries[i];
		...
		{
//			if ( m_UnlimitedHairDye.Deleted )
//				return;

			Mobile m = from.Mobile;
			int[] switches = info.Switches;

			if ( !m_UnlimitedHairDye.IsChildOf( m.Backpack ) ) 
			{
			...
			if ( info.ButtonID != 0 && switches.Length > 0 )
			{
				...
				else
				{
					...
					{
						UnlimitedHairDyeEntry e = m_Entries[entryIndex];
