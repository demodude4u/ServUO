...
{
	public class UnlimitedBeardDye : Item
	{
		public override int LabelNumber{ get{ return 1041060; } } // Hair Dye

		[Constructable]
		public UnlimitedBeardDye() : base( 0xEFF )
		{
			Name = "Beard Dye [Unlimited Use]";
			Weight = 1.0;
		}

		public UnlimitedBeardDye( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.CloseGump( typeof( UnlimitedBeardDyeGump ) );
				from.SendGump( new UnlimitedBeardDyeGump( this ) );
			}
	...
	public class UnlimitedBeardDyeGump : Gump
	{
		private UnlimitedBeardDye m_UnlimitedBeardDye;

		private class UnlimitedBeardDyeEntry
		{
			...
			public UnlimitedBeardDyeEntry( string name, int hueStart, int hueCount )
			{
				m_Name = name;
				m_HueStart = hueStart;
				m_HueCount = hueCount;
		...
		private static UnlimitedBeardDyeEntry[] m_Entries = new UnlimitedBeardDyeEntry[]
			{
				new UnlimitedBeardDyeEntry( "*****", 1602, 26 ),
				new UnlimitedBeardDyeEntry( "*****", 1628, 27 ),
				new UnlimitedBeardDyeEntry( "*****", 1502, 32 ),
				new UnlimitedBeardDyeEntry( "*****", 1302, 32 ),
				new UnlimitedBeardDyeEntry( "*****", 1402, 32 ),
				new UnlimitedBeardDyeEntry( "*****", 1202, 24 ),
				new UnlimitedBeardDyeEntry( "*****", 2402, 29 ),
				new UnlimitedBeardDyeEntry( "*****", 2213, 6 ),
				new UnlimitedBeardDyeEntry( "*****", 1102, 8 ),
				new UnlimitedBeardDyeEntry( "*****", 1110, 8 ),
				new UnlimitedBeardDyeEntry( "*****", 1118, 16 ),
				new UnlimitedBeardDyeEntry( "*****", 1134, 16 )
			};

		public UnlimitedBeardDyeGump( UnlimitedBeardDye dye ) : base( 50, 50 )
		{
			m_UnlimitedBeardDye = dye;
			...
			for ( int i = 0; i < m_Entries.Length; ++i )
			{
				UnlimitedBeardDyeEntry e = m_Entries[i];
		...
		{
//			if ( m_UnlimitedBeardDye.Deleted )
//				return;

			Mobile m = from.Mobile;
			int[] switches = info.Switches;

			if ( !m_UnlimitedBeardDye.IsChildOf( m.Backpack ) ) 
			{
			...
			if ( info.ButtonID != 0 && switches.Length > 0 )
			{
				...
				else
				{
					...
					{
						UnlimitedBeardDyeEntry e = m_Entries[entryIndex];
