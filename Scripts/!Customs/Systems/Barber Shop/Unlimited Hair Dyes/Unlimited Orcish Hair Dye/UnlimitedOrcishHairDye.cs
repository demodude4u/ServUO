...
{
    public class UnlimitedOrcishHairDye : Item
    {
        [Constructable]
        public UnlimitedOrcishHairDye()
            : base(0x42B4)
        {
	        Name = "Orcish Hair Dye [Unlimited Use]";
	        Hue = 2212;
            this.Weight = 1.0;
            this.LootType = LootType.Newbied;
        }

        public UnlimitedOrcishHairDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(UnlimitedOrcishHairDyeGump));
                from.SendGump(new UnlimitedOrcishHairDyeGump(this));
            }
            else
            {
    ...
    public class UnlimitedOrcishHairDyeGump : Gump
    {
        private static readonly UnlimitedOrcishHairDyeEntry[] m_Entries = new UnlimitedOrcishHairDyeEntry[]
        {
            new UnlimitedOrcishHairDyeEntry("Pinks", 0x4B1, 25),
            new UnlimitedOrcishHairDyeEntry("Blues", 0x515, 25),
            new UnlimitedOrcishHairDyeEntry("Greens", 0x579, 25),
            new UnlimitedOrcishHairDyeEntry("Oranges", 0x5DD, 25),
            new UnlimitedOrcishHairDyeEntry("Reds", 0x641, 25),
            new UnlimitedOrcishHairDyeEntry("Yellows", 0x6A5, 25)
        };
        private readonly UnlimitedOrcishHairDye m_UnlimitedOrcishHairDye;
        public UnlimitedOrcishHairDyeGump(UnlimitedOrcishHairDye dye)
            : base(0, 0)
        {
            this.m_UnlimitedOrcishHairDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                UnlimitedOrcishHairDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_UnlimitedOrcishHairDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_UnlimitedOrcishHairDye.IsChildOf(m.Backpack)) 
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        UnlimitedOrcishHairDyeEntry e = m_Entries[entryIndex];

                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
//							this.m_UnlimitedOrcishHairDye.Delete();
        ...
        private class UnlimitedOrcishHairDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public UnlimitedOrcishHairDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
