...
{
    public class UnlimitedHumanHairDye : Item
    {
        [Constructable]
        public UnlimitedHumanHairDye()
            : base(0x1848)
        {
	        Name = "Human Hair Dye [Unlimited Use]";
            this.Weight = 1.0;
            this.LootType = LootType.Newbied;
        }

        public UnlimitedHumanHairDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(UnlimitedHumanHairDyeGump));
                from.SendGump(new UnlimitedHumanHairDyeGump(this));
            }
            else
            {
    ...
    public class UnlimitedHumanHairDyeGump : Gump
    {
        private static readonly UnlimitedHumanHairDyeEntry[] m_Entries = new UnlimitedHumanHairDyeEntry[]
        {
            new UnlimitedHumanHairDyeEntry("Human 1", 0x44E, 24),
            new UnlimitedHumanHairDyeEntry("Human 2", 0x466, 24)
        };
        private readonly UnlimitedHumanHairDye m_UnlimitedHumanHairDye;
        public UnlimitedHumanHairDyeGump(UnlimitedHumanHairDye dye)
            : base(0, 0)
        {
            this.m_UnlimitedHumanHairDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                UnlimitedHumanHairDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_UnlimitedHumanHairDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_UnlimitedHumanHairDye.IsChildOf(m.Backpack)) 
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        UnlimitedHumanHairDyeEntry e = m_Entries[entryIndex];

                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
//							this.m_UnlimitedHumanHairDye.Delete();
        ...
        private class UnlimitedHumanHairDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public UnlimitedHumanHairDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
