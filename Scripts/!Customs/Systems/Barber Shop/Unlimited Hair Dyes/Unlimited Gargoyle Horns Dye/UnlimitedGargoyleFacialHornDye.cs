...
{
    public class UnlimitedGargoyleFacialHornDye : Item
    {
        [Constructable]
        public UnlimitedGargoyleFacialHornDye()
            : base(0x42B4)
        {
	        Name = "Gargoyle Facial Horn Dye [Unlimited Use]";
            this.Weight = 1.0;
            this.LootType = LootType.Newbied;
        }

        public UnlimitedGargoyleFacialHornDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(UnlimitedGargoyleFacialHornDyeGump));
                from.SendGump(new UnlimitedGargoyleFacialHornDyeGump(this));
            }
            else
            {
    ...
    public class UnlimitedGargoyleFacialHornDyeGump : Gump
    {
        private static readonly UnlimitedGargoyleFacialHornDyeEntry[] m_Entries = new UnlimitedGargoyleFacialHornDyeEntry[]
        {
            new UnlimitedGargoyleFacialHornDyeEntry("Browns", 0x709, 10),
            new UnlimitedGargoyleFacialHornDyeEntry("Grays", 0x765, 10),
            new UnlimitedGargoyleFacialHornDyeEntry("Purples", 0x6E0, 10),
            new UnlimitedGargoyleFacialHornDyeEntry("Reds", 0x6EA, 10),
        };
        private readonly UnlimitedGargoyleFacialHornDye m_UnlimitedGargoyleFacialHornDye;
        public UnlimitedGargoyleFacialHornDyeGump(UnlimitedGargoyleFacialHornDye dye)
            : base(0, 0)
        {
            this.m_UnlimitedGargoyleFacialHornDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                UnlimitedGargoyleFacialHornDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_UnlimitedGargoyleFacialHornDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_UnlimitedGargoyleFacialHornDye.IsChildOf(m.Backpack))
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        UnlimitedGargoyleFacialHornDyeEntry e = m_Entries[entryIndex];
...
//							this.m_UnlimitedGargoyleFacialHornDye.Delete();
                            m.PlaySound(0x4E);
                        }
                    }
                }
        ...
        private class UnlimitedGargoyleFacialHornDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public UnlimitedGargoyleFacialHornDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
