...
{
    public class UnlimitedGargoyleHornDye : Item
    {
        [Constructable]
        public UnlimitedGargoyleHornDye()
            : base(0x42B4)
        {
	        Name = "Gargoyle Horn Dye [Unlimited Use]";
            this.Weight = 1.0;
            this.LootType = LootType.Newbied;
        }

        public UnlimitedGargoyleHornDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(UnlimitedGargoyleHornDyeGump));
                from.SendGump(new UnlimitedGargoyleHornDyeGump(this));
            }
            else
            {
    ...
    public class UnlimitedGargoyleHornDyeGump : Gump
    {
        private static readonly UnlimitedGargoyleHornDyeEntry[] m_Entries = new UnlimitedGargoyleHornDyeEntry[]
        {
            new UnlimitedGargoyleHornDyeEntry("Gargish Browns", 0x709, 10),
            new UnlimitedGargoyleHornDyeEntry("Gargish Grays", 0x765, 10),
            new UnlimitedGargoyleHornDyeEntry("Gargish Purples", 0x6E0, 10),
            new UnlimitedGargoyleHornDyeEntry("Gargish Reds", 0x6EA, 10)
        };
        private readonly UnlimitedGargoyleHornDye m_UnlimitedGargoyleHornDye;
        public UnlimitedGargoyleHornDyeGump(UnlimitedGargoyleHornDye dye)
            : base(0, 0)
        {
            this.m_UnlimitedGargoyleHornDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                UnlimitedGargoyleHornDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_UnlimitedGargoyleHornDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_UnlimitedGargoyleHornDye.IsChildOf(m.Backpack)) 
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        UnlimitedGargoyleHornDyeEntry e = m_Entries[entryIndex];

                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
//							this.m_UnlimitedGargoyleHornDye.Delete();
        ...
        private class UnlimitedGargoyleHornDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public UnlimitedGargoyleHornDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
