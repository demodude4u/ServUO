...
{
    public class HumanHairDye : Item
    {
        [Constructable]
        public HumanHairDye()
            : base(0x1848)
        ...
        public HumanHairDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(HumanHairDyeGump));
                from.SendGump(new HumanHairDyeGump(this));
            }
            else
            {
    ...
    public class HumanHairDyeGump : Gump
    {
        private static readonly HumanHairDyeEntry[] m_Entries = new HumanHairDyeEntry[]
        {
            new HumanHairDyeEntry("Human 1", 0x44E, 24),
            new HumanHairDyeEntry("Human 2", 0x466, 24)
        };
        private readonly HumanHairDye m_HumanHairDye;
        public HumanHairDyeGump(HumanHairDye dye)
            : base(0, 0)
        {
            this.m_HumanHairDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                HumanHairDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_HumanHairDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_HumanHairDye.IsChildOf(m.Backpack)) 
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        HumanHairDyeEntry e = m_Entries[entryIndex];

                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
							this.m_HumanHairDye.Delete();
        ...
        private class HumanHairDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public HumanHairDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
