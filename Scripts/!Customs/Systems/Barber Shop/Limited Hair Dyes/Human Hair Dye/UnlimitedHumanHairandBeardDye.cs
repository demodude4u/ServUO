...
{
    public class HumanHairandBeardDye : Item
    {
        [Constructable]
        public HumanHairandBeardDye()
            : base(0x1848)
        {
	        Name = "Human Hair and Beard Dye";
	        Hue = 51;
            this.Weight = 1.0;
        }

        public HumanHairandBeardDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(HumanHairandBeardDyeGump));
                from.SendGump(new HumanHairandBeardDyeGump(this));
            }
            else
            {
    ...
    public class HumanHairandBeardDyeGump : Gump
    {
        private static readonly HumanHairandBeardDyeEntry[] m_Entries = new HumanHairandBeardDyeEntry[]
        {
            new HumanHairandBeardDyeEntry("Human 1", 0x44E, 24),
            new HumanHairandBeardDyeEntry("Human 2", 0x466, 24)
        };
        private readonly HumanHairandBeardDye m_HumanHairandBeardDye;
        public HumanHairandBeardDyeGump(HumanHairandBeardDye dye)
            : base(50, 50)
        {
            this.m_HumanHairandBeardDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                HumanHairandBeardDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_HumanHairandBeardDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_HumanHairandBeardDye.IsChildOf(m.Backpack)) 
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        HumanHairandBeardDyeEntry e = m_Entries[entryIndex];
                        ...
                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
							...
							this.m_HumanHairandBeardDye.Delete();
                            m.PlaySound(0x4E);
                        }
                    }
                }
        ...
        private class HumanHairandBeardDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public HumanHairandBeardDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
