...
{
    public class OrcishBeardDye : Item
    {
        [Constructable]
        public OrcishBeardDye()
            : base(0x42B4)
        {
	        Name = "Orcish Beard Dye";
	        Hue = 2213;
            this.Weight = 1.0;
            this.LootType = LootType.Newbied;
        }

        public OrcishBeardDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(OrcishBeardDyeGump));
                from.SendGump(new OrcishBeardDyeGump(this));
            }
            else
            {
    ...
    public class OrcishBeardDyeGump : Gump
    {
        private static readonly OrcishBeardDyeEntry[] m_Entries = new OrcishBeardDyeEntry[]
        {
            new OrcishBeardDyeEntry("Pinks", 0x4B1, 25),
            new OrcishBeardDyeEntry("Blues", 0x515, 25),
            new OrcishBeardDyeEntry("Greens", 0x579, 25),
            new OrcishBeardDyeEntry("Oranges", 0x5DD, 25),
            new OrcishBeardDyeEntry("Reds", 0x641, 25),
            new OrcishBeardDyeEntry("Yellows", 0x6A5, 25)
        };
        private readonly OrcishBeardDye m_OrcishBeardDye;
        public OrcishBeardDyeGump(OrcishBeardDye dye)
            : base(0, 0)
        {
            this.m_OrcishBeardDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                OrcishBeardDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_OrcishBeardDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_OrcishBeardDye.IsChildOf(m.Backpack))
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        OrcishBeardDyeEntry e = m_Entries[entryIndex];
        ...
        private class OrcishBeardDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public OrcishBeardDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
