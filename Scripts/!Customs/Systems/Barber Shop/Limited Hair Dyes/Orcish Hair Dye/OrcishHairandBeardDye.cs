...
{
    public class OrcishHairandBeardDye : Item
    {
        [Constructable]
        public OrcishHairandBeardDye()
            : base(0x42B4)
        {
	        Name = "Orcish Hair and Beard Dye";
	        Hue = 2213;
            this.Weight = 1.0;
        }

        public OrcishHairandBeardDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(OrcishHairandBeardDyeGump));
                from.SendGump(new OrcishHairandBeardDyeGump(this));
            }
            else
            {
    ...
    public class OrcishHairandBeardDyeGump : Gump
    {
        private static readonly OrcishHairandBeardDyeEntry[] m_Entries = new OrcishHairandBeardDyeEntry[]
        {
            new OrcishHairandBeardDyeEntry("Pinks", 0x4B1, 25),
            new OrcishHairandBeardDyeEntry("Blues", 0x515, 25),
            new OrcishHairandBeardDyeEntry("Greens", 0x579, 25),
            new OrcishHairandBeardDyeEntry("Oranges", 0x5DD, 25),
            new OrcishHairandBeardDyeEntry("Reds", 0x641, 25),
            new OrcishHairandBeardDyeEntry("Yellows", 0x6A5, 25)
        };
        private readonly OrcishHairandBeardDye m_OrcishHairandBeardDye;
        public OrcishHairandBeardDyeGump(OrcishHairandBeardDye dye)
            : base(50, 50)
        {
            this.m_OrcishHairandBeardDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                OrcishHairandBeardDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_OrcishHairandBeardDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_OrcishHairandBeardDye.IsChildOf(m.Backpack)) 
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        OrcishHairandBeardDyeEntry e = m_Entries[entryIndex];
                        ...
                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
							...
							this.m_OrcishHairandBeardDye.Delete();
                            m.PlaySound(0x4E);
                        }
                    }
                }
        ...
        private class OrcishHairandBeardDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public OrcishHairandBeardDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
