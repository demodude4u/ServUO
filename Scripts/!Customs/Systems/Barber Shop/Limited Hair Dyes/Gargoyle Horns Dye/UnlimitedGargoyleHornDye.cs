...
{
    public class GargoyleHornDye : Item
    {
        [Constructable]
        public GargoyleHornDye()
            : base(0x42B4)
        {
	        Name = "Gargoyle Horn Dye";
            this.Weight = 1.0;
            this.LootType = LootType.Newbied;
        }

        public GargoyleHornDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(GargoyleHornDyeGump));
                from.SendGump(new GargoyleHornDyeGump(this));
            }
            else
            {
    ...
    public class GargoyleHornDyeGump : Gump
    {
        private static readonly GargoyleHornDyeEntry[] m_Entries = new GargoyleHornDyeEntry[]
        {
            new GargoyleHornDyeEntry("Browns", 0x709, 10),
            new GargoyleHornDyeEntry("Grays", 0x765, 10),
            new GargoyleHornDyeEntry("Purples", 0x6E0, 10),
            new GargoyleHornDyeEntry("Reds", 0x6EA, 10)
        };
        private readonly GargoyleHornDye m_GargoyleHornDye;
        public GargoyleHornDyeGump(GargoyleHornDye dye)
            : base(0, 0)
        {
            this.m_GargoyleHornDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                GargoyleHornDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_GargoyleHornDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_GargoyleHornDye.IsChildOf(m.Backpack)) 
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        GargoyleHornDyeEntry e = m_Entries[entryIndex];

                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
							this.m_GargoyleHornDye.Delete();
        ...
        private class GargoyleHornDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public GargoyleHornDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
