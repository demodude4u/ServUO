...
{
    public class GargoyleFacialHornDye : Item
    {
        [Constructable]
        public GargoyleFacialHornDye()
            : base(0x42B4)
        {
	        Name = "Gargoyle Facial Horn Dye";
            this.Weight = 1.0;
            this.LootType = LootType.Newbied;
        }

        public GargoyleFacialHornDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(GargoyleFacialHornDyeGump));
                from.SendGump(new GargoyleFacialHornDyeGump(this));
            }
            else
            {
    ...
    public class GargoyleFacialHornDyeGump : Gump
    {
        private static readonly GargoyleFacialHornDyeEntry[] m_Entries = new GargoyleFacialHornDyeEntry[]
        {
            new GargoyleFacialHornDyeEntry("Browns", 0x709, 10),
            new GargoyleFacialHornDyeEntry("Grays", 0x765, 10),
            new GargoyleFacialHornDyeEntry("Purples", 0x6E0, 10),
            new GargoyleFacialHornDyeEntry("Reds", 0x6EA, 10),
        };
        private readonly GargoyleFacialHornDye m_GargoyleFacialHornDye;
        public GargoyleFacialHornDyeGump(GargoyleFacialHornDye dye)
            : base(0, 0)
        {
            this.m_GargoyleFacialHornDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                GargoyleFacialHornDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_GargoyleFacialHornDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_GargoyleFacialHornDye.IsChildOf(m.Backpack))
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        GargoyleFacialHornDyeEntry e = m_Entries[entryIndex];
                        ...
                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
							...
							this.m_GargoyleFacialHornDye.Delete();
                            m.PlaySound(0x4E);
                        }
                    }
                }
        ...
        private class GargoyleFacialHornDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public GargoyleFacialHornDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
