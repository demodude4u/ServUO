...
{
	public class BeardDye : Item
	{
		public override int LabelNumber{ get{ return 1041060; } } // Hair Dye

		[Constructable]
		public BeardDye() : base( 0xEFF )
		{
			Name = "Beard Dye";
			Weight = 1.0;
		}

		public BeardDye( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.CloseGump( typeof( BeardDyeGump ) );
				from.SendGump( new BeardDyeGump( this ) );
			}
	...
	public class BeardDyeGump : Gump
	{
		private BeardDye m_BeardDye;

		private class BeardDyeEntry
		{
			...
			public BeardDyeEntry( string name, int hueStart, int hueCount )
			{
				m_Name = name;
				m_HueStart = hueStart;
				m_HueCount = hueCount;
		...
		private static BeardDyeEntry[] m_Entries = new BeardDyeEntry[]
			{
				new BeardDyeEntry( "*****", 1602, 26 ),
				new BeardDyeEntry( "*****", 1628, 27 ),
				new BeardDyeEntry( "*****", 1502, 32 ),
				new BeardDyeEntry( "*****", 1302, 32 ),
				new BeardDyeEntry( "*****", 1402, 32 ),
				new BeardDyeEntry( "*****", 1202, 24 ),
				new BeardDyeEntry( "*****", 2402, 29 ),
				new BeardDyeEntry( "*****", 2213, 6 ),
				new BeardDyeEntry( "*****", 1102, 8 ),
				new BeardDyeEntry( "*****", 1110, 8 ),
				new BeardDyeEntry( "*****", 1118, 16 ),
				new BeardDyeEntry( "*****", 1134, 16 )
			};

		public BeardDyeGump( BeardDye dye ) : base( 50, 50 )
		{
			m_BeardDye = dye;
		...
		{
			if ( m_BeardDye.Deleted )
				return;

			Mobile m = from.Mobile;
			int[] switches = info.Switches;

			if ( !m_BeardDye.IsChildOf( m.Backpack ) ) 
			{
			...
			if ( info.ButtonID != 0 && switches.Length > 0 )
			{
				...
				else
				{
					...
					{
						BeardDyeEntry e = m_Entries[entryIndex];
