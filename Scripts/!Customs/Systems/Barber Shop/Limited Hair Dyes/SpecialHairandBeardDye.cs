...
{
    public class SpecialHairandBeardDye : Item
    {
        [Constructable]
        public SpecialHairandBeardDye()
            : base(0xE26)
        {
            this.Weight = 1.0;
        ...
        public SpecialHairandBeardDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override string DefaultName
        {
            get
            {
                return "Special Hair and Beard Dye";
            }
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(SpecialHairandBeardDyeGump));
                from.SendGump(new SpecialHairandBeardDyeGump(this));
            }
            else
            {
    ...
    public class SpecialHairandBeardDyeGump : Gump
    {
        private static readonly SpecialHairandBeardDyeEntry[] m_Entries = new SpecialHairandBeardDyeEntry[]
        {
            new SpecialHairandBeardDyeEntry("*****", 12, 10),
            new SpecialHairandBeardDyeEntry("*****", 32, 5),
            new SpecialHairandBeardDyeEntry("*****", 38, 8),
            new SpecialHairandBeardDyeEntry("*****", 54, 3),
            new SpecialHairandBeardDyeEntry("*****", 62, 10),
            new SpecialHairandBeardDyeEntry("*****", 81, 2),
            new SpecialHairandBeardDyeEntry("*****", 89, 2),
            new SpecialHairandBeardDyeEntry("*****", 1153, 2)
        };
        private readonly SpecialHairandBeardDye m_SpecialHairandBeardDye;
        public SpecialHairandBeardDyeGump(SpecialHairandBeardDye dye)
            : base(0, 0)
        {
            this.m_SpecialHairandBeardDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                SpecialHairandBeardDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_SpecialHairandBeardDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_SpecialHairandBeardDye.IsChildOf(m.Backpack)) 
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        SpecialHairandBeardDyeEntry e = m_Entries[entryIndex];

                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
                            this.m_SpecialHairandBeardDye.Delete();
        ...
        private class SpecialHairandBeardDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public SpecialHairandBeardDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
