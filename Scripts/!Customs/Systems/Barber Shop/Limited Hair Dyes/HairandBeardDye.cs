...
{
	public class HairandBeardDye : Item
	{
		public override int LabelNumber{ get{ return 1041060; } } // Hair Dye

		[Constructable]
		public HairandBeardDye() : base( 0xEFF )
		{
			Name = "Hair and Beard Dye";
			Weight = 1.0;
		}

		public HairandBeardDye( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.CloseGump( typeof( HairandBeardDyeGump ) );
				from.SendGump( new HairandBeardDyeGump( this ) );
			}
	...
	public class HairandBeardDyeGump : Gump
	{
		private HairandBeardDye m_HairandBeardDye;

		private class HairandBeardDyeEntry
		{
			...
			public HairandBeardDyeEntry( string name, int hueStart, int hueCount )
			{
				m_Name = name;
				m_HueStart = hueStart;
				m_HueCount = hueCount;
		...
		private static HairandBeardDyeEntry[] m_Entries = new HairandBeardDyeEntry[]
			{
				new HairandBeardDyeEntry( "*****", 1602, 26 ),
				new HairandBeardDyeEntry( "*****", 1628, 27 ),
				new HairandBeardDyeEntry( "*****", 1502, 32 ),
				new HairandBeardDyeEntry( "*****", 1302, 32 ),
				new HairandBeardDyeEntry( "*****", 1402, 32 ),
				new HairandBeardDyeEntry( "*****", 1202, 24 ),
				new HairandBeardDyeEntry( "*****", 2402, 29 ),
				new HairandBeardDyeEntry( "*****", 2213, 6 ),
				new HairandBeardDyeEntry( "*****", 1102, 8 ),
				new HairandBeardDyeEntry( "*****", 1110, 8 ),
				new HairandBeardDyeEntry( "*****", 1118, 16 ),
				new HairandBeardDyeEntry( "*****", 1134, 16 )
			};

		public HairandBeardDyeGump( HairandBeardDye dye ) : base( 50, 50 )
		{
			m_HairandBeardDye = dye;
			...
			for ( int i = 0; i < m_Entries.Length; ++i )
			{
				HairandBeardDyeEntry e = m_Entries[i];
		...
		{
			if ( m_HairandBeardDye.Deleted )
				return;

			Mobile m = from.Mobile;
			int[] switches = info.Switches;

			if ( !m_HairandBeardDye.IsChildOf( m.Backpack ) ) 
			{
			...
			if ( info.ButtonID != 0 && switches.Length > 0 )
			{
				...
				else
				{
					...
					{
						HairandBeardDyeEntry e = m_Entries[entryIndex];
