...
{
    public class ElvenHairandBeardDye : Item
    {
        [Constructable]
        public ElvenHairandBeardDye()
            : base(0x1847)
        {
	        Name = "Elven Hair and Beard Dye";
            this.Weight = 1.0;
        }

        public ElvenHairandBeardDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(ElvenHairandBeardDyeGump));
                from.SendGump(new ElvenHairandBeardDyeGump(this));
            }
            else
            {
    ...
    public class ElvenHairandBeardDyeGump : Gump
    {
        private static readonly ElvenHairandBeardDyeEntry[] m_Entries = new ElvenHairandBeardDyeEntry[]
        {
            new ElvenHairandBeardDyeEntry("Elven Hair 1", 0x034, 10),
            new ElvenHairandBeardDyeEntry("Elven Hair 2", 0x058, 10),
            new ElvenHairandBeardDyeEntry("Elven Hair 3", 0x08e, 10),
            new ElvenHairandBeardDyeEntry("Elven Hair 4", 0x090, 10),
            new ElvenHairandBeardDyeEntry("Elven Hair 5", 0x101, 10),
            new ElvenHairandBeardDyeEntry("Elven Hair 6", 0x159, 10),
            new ElvenHairandBeardDyeEntry("Elven Hair 7", 0x15A, 10),
            new ElvenHairandBeardDyeEntry("Elven Hair 8", 0x320, 10)
        };
        private readonly ElvenHairandBeardDye m_ElvenHairandBeardDye;
        public ElvenHairandBeardDyeGump(ElvenHairandBeardDye dye)
            : base(50, 50)
        {
            this.m_ElvenHairandBeardDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                ElvenHairandBeardDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_ElvenHairandBeardDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_ElvenHairandBeardDye.IsChildOf(m.Backpack)) 
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        ElvenHairandBeardDyeEntry e = m_Entries[entryIndex];
                        ...
                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
							...
							this.m_ElvenHairandBeardDye.Delete();
                            m.PlaySound(0x4E);
                        }
                    }
                }
        ...
        private class ElvenHairandBeardDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public ElvenHairandBeardDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
