...
{
    public class ElvenHairDye : Item
    {
        [Constructable]
        public ElvenHairDye()
            : base(0x1847)
        ...
        public ElvenHairDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(ElvenHairDyeGump));
                from.SendGump(new ElvenHairDyeGump(this));
            }
            else
            {
    ...
    public class ElvenHairDyeGump : Gump
    {
        private static readonly ElvenHairDyeEntry[] m_Entries = new ElvenHairDyeEntry[]
        {
            new ElvenHairDyeEntry("Elven Hair 1", 0x034, 10),
            new ElvenHairDyeEntry("Elven Hair 2", 0x058, 10),
            new ElvenHairDyeEntry("Elven Hair 3", 0x08e, 10),
            new ElvenHairDyeEntry("Elven Hair 4", 0x090, 10),
            new ElvenHairDyeEntry("Elven Hair 5", 0x101, 10),
            new ElvenHairDyeEntry("Elven Hair 6", 0x159, 10),
            new ElvenHairDyeEntry("Elven Hair 7", 0x15A, 10),
            new ElvenHairDyeEntry("Elven Hair 8", 0x320, 10)
        };
        private readonly ElvenHairDye m_ElvenHairDye;
        public ElvenHairDyeGump(ElvenHairDye dye)
            : base(0, 0)
        {
            this.m_ElvenHairDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                ElvenHairDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_ElvenHairDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_ElvenHairDye.IsChildOf(m.Backpack)) 
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        ElvenHairDyeEntry e = m_Entries[entryIndex];

                        if (hueOffset >= 0 && hueOffset < e.HueCount)
                        {
							this.m_ElvenHairDye.Delete();
        ...
        private class ElvenHairDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public ElvenHairDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
