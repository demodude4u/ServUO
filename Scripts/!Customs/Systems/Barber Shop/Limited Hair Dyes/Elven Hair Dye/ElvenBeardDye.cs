...
{
    public class ElvenBeardDye : Item
    {
        [Constructable]
        public ElvenBeardDye()
            : base(0x1847)
        {
	        Name = "Elven Beard Dye";
            this.Weight = 1.0;
            this.LootType = LootType.Newbied;
        }

        public ElvenBeardDye(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.CloseGump(typeof(ElvenBeardDyeGump));
                from.SendGump(new ElvenBeardDyeGump(this));
            }
            else
            {
    ...
    public class ElvenBeardDyeGump : Gump
    {
        private static readonly ElvenBeardDyeEntry[] m_Entries = new ElvenBeardDyeEntry[]
        {
            new ElvenBeardDyeEntry("Elven Hair 1", 0x034, 10),
            new ElvenBeardDyeEntry("Elven Hair 2", 0x058, 10),
            new ElvenBeardDyeEntry("Elven Hair 3", 0x08e, 10),
            new ElvenBeardDyeEntry("Elven Hair 4", 0x090, 10),
            new ElvenBeardDyeEntry("Elven Hair 5", 0x101, 10),
            new ElvenBeardDyeEntry("Elven Hair 6", 0x159, 10),
            new ElvenBeardDyeEntry("Elven Hair 7", 0x15A, 10),
            new ElvenBeardDyeEntry("Elven Hair 8", 0x320, 10)
        };
        private readonly ElvenBeardDye m_ElvenBeardDye;
        public ElvenBeardDyeGump(ElvenBeardDye dye)
            : base(0, 0)
        {
            this.m_ElvenBeardDye = dye;
            ...
            for (int i = 0; i < m_Entries.Length; ++i)
            {
                ElvenBeardDyeEntry e = m_Entries[i];
        ...
        {
            if (this.m_ElvenBeardDye.Deleted)
                return;

            Mobile m = from.Mobile;
            int[] switches = info.Switches;

            if (!this.m_ElvenBeardDye.IsChildOf(m.Backpack))
            {
            ...
            if (info.ButtonID != 0 && switches.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        ElvenBeardDyeEntry e = m_Entries[entryIndex];
        ...
        private class ElvenBeardDyeEntry
        {
            private readonly string m_Name;
            private readonly int m_HueStart;
            private readonly int m_HueCount;
            public ElvenBeardDyeEntry(string name, int hueStart, int hueCount)
            {
                this.m_Name = name;
